...
export const RegisterPage = ({ onBack, onTerms, onSubmitSuccess }: { onBack: () => void, onTerms: () => void, onSubmitSuccess: () => void }) => {
  ...
  return (
    <div className="min-h-[100dvh] bg-[#F8F8F6] font-sans pb-10 flex flex-col relative overflow-hidden">

      {/* Header Area */}
      <div className="relative z-10 px-6 pt-10 pb-4 flex items-center mb-4 border-b border-[#EAEAE6]">
        <button type="button" onClick={handlePrev} className="bg-[#FFFFFF] rounded-full w-10 h-10 flex items-center justify-center border border-[#EAEAE6] cursor-pointer hover:bg-[#F2F2F0] transition-colors shrink-0">
          <Ic n="back" color="#2D2D2A" size={20} />
        </button>
        <div className="flex-1 flex flex-col items-center justify-center -ml-10">
            <h1 className="text-[14px] font-serif font-bold text-[#2D2D2A] tracking-[0.2em] uppercase">會員註冊</h1>
        </div>
      </div>
