...
// Map Notion row fields to frontend coverage category format
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (response.results.length === 0) {
      // Return empty structure when no coverage records found
      return {
        statusCode: 200,
    ...
    // Map each Notion row (one per family member) to frontend format
